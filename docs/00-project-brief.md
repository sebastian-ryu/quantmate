...
조건 검색, 퀀트 전략, 자동매매 전략은 별도 개념으로 흩뜨리지 않고 하나의 전략 정의에서 출발한다. 같은 전략 정의를 `research`, `backtest`, `paper`, `live-trading` 실행 모드가 공유하되, 실제 주문을 내는 실행 계층은 주문 한도, 중복 주문 방지, 체결 확인, 손실 제한, 긴급 중지 같은 안전장치를 별도로 가진다.
