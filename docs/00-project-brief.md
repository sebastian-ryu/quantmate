...
첫 번째로 쓸 수 있는 제품은 설명 가능한 알고리즘으로 종목을 추천하고, 백테스트로 검증할 수 있어야 한다. 한국투자증권 모의투자와 실거래 자동 매매는 이후 단계이며 강한 안전장치가 필요하다.
...
6. 매매 연동 전에 전략을 백테스트한다.
7. 한국투자증권 Open API로 실시간 데이터와 향후 매매 기능을 연동한다.
8. 한국투자증권 모의투자 계좌 연동을 실거래 전 검증 단계로 둔다.
9. 한국 주식을 먼저 지원한다.
