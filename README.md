...
현재 저장소는 검색기, 전략, 백테스트 중심의 로컬 웹 UI와 기본 전략 카탈로그, 전략 후보 실행 API, 서버 백테스트 실행 API, 사용자 등록 전략 DB 저장, 백테스트 결과 저장/조회, 로컬 MySQL 스키마를 구현한 단계입니다. 전략 후보와 백테스트 결과는 아직 샘플 데이터 기반이며, 실제 한국 주식 데이터 연결은 공식 KRX Open API 인증키 방식으로 단계적으로 붙이고 있습니다.
