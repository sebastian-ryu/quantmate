...
`PUBLIC_SHOW_PAPER_TRADING_UI=true`는 전략 화면에서 KIS 모의투자 계좌, 주문 제안, 모의 일괄 주문 영역을 표시한다. 기본값은 `false`이며, 설정이 누락되면 화면에는 모의투자 영역을 보이지 않는다. `PAPER_TRADING_ENABLED=false`는 화면 표시 여부와 별개인 서버 주문 제출 잠금이다. 모의주문 제출은 이 값을 `true`로 바꾸고, 요청 본문에 `confirm_submit=true`를 넣은 경우에만 동작한다.
...
초기 장기 일봉 적재는 `최신 데이터 갱신` 버튼이 아니라 `scripts/krx-backfill-daily-prices.sh`를 사용한다. 기본값은 `2010년~현재`, `KOSPI KOSDAQ`이며, KRX 시장 일봉 API를 월별 시장 단위로 나누어 호출한다. 스크립트는 실행 전 전체 단계 수를 계산하고, 각 월/시장 구간의 DB 커버리지를 조회해 이미 충분히 적재된 구간은 건너뛴다.

백테스트 화면의 시작연도도 KRX 공식 장기 일봉 적재 범위에 맞춰 2010년부터 선택할 수 있게 제한한다. Yahoo/yfinance가 일부 종목에서 더 오래된 데이터를 줄 수 있더라도, 한국 주식 전체 종목 선정과 동적 리밸런싱 백테스트는 KRX/KIS 종목 마스터와 DB 일봉 커버리지가 함께 필요하므로 2010년을 신뢰 가능한 최소 기준으로 둔다.

초기 보조 데이터 적재는 `scripts/initial-enrichment.py`를 사용한다. 이 스크립트는 DB에 저장된 종목 마스터를 읽어 KIS 현재가 스냅샷, KIS 재무비율, OpenDART 재무요약을 종목 단위로 1회 적재하고, KIS 투자자별 매매동향/공매도/신용잔고는 월별 종목 단위로 적재한다. 기본 기간은 최근 1년이며, `ENRICHMENT_START_DATE`, `ENRICHMENT_END_DATE`, `ENRICHMENT_MARKETS`, `ENRICHMENT_SYMBOL_LIMIT`로 범위를 줄일 수 있다. 실행 중에는 `[현재/전체 완료율]` 형식으로 진행률을 출력하고, 실패 구간은 마지막에 요약한다. 재실행 시에는 `/api/data/enrichment/coverage`가 DB 커버리지를 확인해 이미 충분히 적재된 종목/월을 건너뛴다.
